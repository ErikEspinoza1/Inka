...
def update_booking(
    ...
    # Actualizar campos permitidos dinámicamente según el rol
    for key, value in update_data.dict(exclude_unset=True).items():
        if key == "client_accepted":
            ...
            if is_artist or (is_client and value is False):
                ...
                raise HTTPException(status_code=403, detail=f"Not allowed to update field: {key}")
        elif key in {"status", "price_quote", "booking_date", "idea_description", "body_part", "size_cm"}:
            if is_artist:
                setattr(booking, key, value)
            else:
                raise HTTPException(status_code=403, detail=f"Not allowed to update field: {key}")
    ...
    # Generación de mensajes automáticos en el chat
